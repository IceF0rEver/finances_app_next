...
export default function SubscriptionManage({ sheetOpen, onSheetOpen, status, data }: SubscriptionManageProps) {
	...
	const [state, FormAction, isPending] = useActionState(status ? updateSubscription : createSubscription, {
		success: false,
	});
					...
					<form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-6 px-4">
						...
						<SubscriptionField
							...
							datas={[
								{
									label: t(
										"app.dashboard.subscription.components.subscriptionManage.form.recurrence.monthly",
									),
									value: "monthly",
								},
								{
									label: t(
										"app.dashboard.subscription.components.subscriptionManage.form.recurrence.annually",
									),
									value: "annually",
								},
							]}
						...
						<SubscriptionField
							label={t(
								"app.dashboard.subscription.components.subscriptionManage.form.executionDate.label",
							)}
							control={form.control}
							name="executionDate"
							fieldType="date"
							...
							<Button type="submit" variant="default" className="w-full" disabled={isPending}>
								{isPending ? (
									...
									<span>
										{status
											? t(
													"app.dashboard.subscription.components.subscriptionManage.button.update",
												)
