...
import type { BudgetCardProps } from "@/app/[locale]/dashboard/budget/_components/_types/budget-types";
...
export default function BudgetCard<T extends FieldValues>({
	...
	return (
		<Card
			className={cn(status ? "border-none shadow-none p-1" : "", "grid gap-4")}
		>
			...
			{type === "income" ? (
						...
						[...fields.filter((f) => f.type === "income")].map((field) => {
							...
							return (
								<div
									className={cn(
										status ? "p-0" : "",
										"grid grid-cols-11 gap-3 mb-4 items-start",
									)}
									key={field.id}
								>
									<div
									...
									>
										<BudgetField
											label={t(
												"budget.components.budgetManage.form.name.label",
											)}
									...
									>
										<BudgetField
											label={t(
												"budget.components.budgetManage.form.amount.label",
											)}
				...
				<CardContent>
					{fields &&
						[
							...fields.filter(
								(f) => f.type === "expense" && f.from === "budget",
							),
						].map((fieldCategory) => {
							...
							return (
								<Card
									className={cn(status ? "mt-2" : "m-2", "p-0 grid gap-1")}
									key={fieldCategory.id}
								>
									<div className="relative">
										<div className="absolute right-0">
											<Button
												...
												variant="secondary"
												onClick={() =>
													onRemove("subCategory", fieldCategory.id)
												}
											>
												<Trash2 />
											</Button>
									...
									>
										...
										>
											<BudgetField
												label={t(
													"budget.components.budgetManage.form.categoryName.label",
												)}
									...
									<CardFooter
										className={cn(
											status ? "p-2" : "flex justify-between px-4 pb-4",
										)}
									>
										<Button
											type="button"
										...
										>
											{t("budget.components.budgetManage.button.addExpense")}
										</Button>
									</CardFooter>
								</Card>
							);
			...
			<CardFooter className={cn(status ? "p-2" : "flex justify-between")}>
				<Button
					...
					variant="secondary"
					onClick={() =>
						type === "income" ? onAdd("income") : onAdd("category")
					}
				>
					{type === "income" ? t("button.addIncome") : t("addCategoryExpense")}
				</Button>
			</CardFooter>
		</Card>
	);
